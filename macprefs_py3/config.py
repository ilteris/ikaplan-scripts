...
def get_preferences_dir():
    return path.join(get_home_dir(), "Library/Preferences/")
...
def get_dotfile_excludes():
    return [
        ".CFUserTextEncoding", ".DS_Store", ".vscode", ".vscode-oss", "node_modules",
        "yarn/global", "gcloud/virtenv", "nvim/lazy", "opencode", ".git",
        "Cache", "Caches", ".rustup", ".cargo", ".npm", ".dropbox"
    ]


def get_preference_excludes():
    return [
        "com.apple.HomeKit*", "com.apple.sidebarlists.plist", "com.apple.finder.plist",
        "Adobe*"
    ]
...
def get_ssh_user_dir():
    return path.join(get_home_dir(), ".ssh/")
...
def get_user_launch_agents_backup_dir():
    return path.join(get_macprefs_dir(), "StartupItems/LaunchAgents/User/")


def get_fonts_dir():
    return path.join(get_home_dir(), "Library/Fonts/")


def get_fonts_backup_dir():
    return_val = path.join(get_macprefs_dir(), "Fonts/")
    ensure_exists(return_val)
    return return_val


def get_app_store_preferences_dir():
    return path.join(get_home_dir(), "Library/Containers/")
...
def get_app_support_dir():
    return path.join(get_home_dir(), 'Library/Application Support/')


def get_app_support_backup_dir():
    return_val = path.join(get_macprefs_dir(), 'ApplicationSupport/')
    ensure_exists(return_val)
    return return_val
